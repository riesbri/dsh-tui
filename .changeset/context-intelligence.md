...
`/context` inspects what the model is currently carrying: provider-anchored
occupancy, the estimated system/tools/messages composition, and the largest
current context entries — each named from the session log, with a bounded
preview. `/compact` stays Harness-owned; `c` inside `/context` dispatches the
