...
- **`/setup`**, which runs by itself on a launch where no provider route is active. It commits a reading of the installation to scrollback — Node, dshline, the Harness generation compared against the one this build targets, the profile, what can configure a provider, and why there is no model — then hands off to `/connect` and, once a route is live, `/model`. It re-reads Harness each pass, stores no first-run state, and writes nothing you did not choose.
