...
const FIRST_WORKSPACE = '/tmp/dsh-tui-sessions-one'
const SECOND_WORKSPACE = '/tmp/dsh-tui-sessions-two'
...
describe('the Sessions catalog over the real session-query engine', () => {
  it('lists the live corpus with the headers Harness recorded', async () => {
    ...
    const first = ctx.sessions.create(SessionId('tui-int-one'), { meta: { cwd: FIRST_WORKSPACE } })
    const second = ctx.sessions.create(SessionId('tui-int-two'), {
  ...
  it('counts a real log through the engine, including its last activity', async () => {
    const ctx = await harness()
    const session = ctx.sessions.create(SessionId('tui-int-detail'), { meta: { cwd: FIRST_WORKSPACE } })
  ...
  it('carries a real search hit and its excerpt into the browser', async () => {
    ...
    const wanted = ctx.sessions.create(SessionId('tui-int-hit'), { meta: { cwd: FIRST_WORKSPACE } })
    const other = ctx.sessions.create(SessionId('tui-int-miss'), { meta: { cwd: SECOND_WORKSPACE } })
  ...
  it('refuses to reopen a session the store still holds live', async () => {
    ...
    const ctx = await harness()
    ctx.sessions.create(SessionId('tui-int-live'), { meta: { cwd: FIRST_WORKSPACE } })
    const catalog = new SessionCatalog({ query: ctx.sessionQuery, invalidate: () => {} })
    catalog.refresh()
    await settled()
    const listing = catalog.listing()
