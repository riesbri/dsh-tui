...
export const name = 'dshline-startup'
...
export function apply(ctx: Context): void {
  ...
  if (exit === undefined) {
    throw new Error('dshline: the launcher must provide ctx.appExit before the tree mounts')
  }
  const program = new Command()
    ...
    .action((task: string[], options: { cwd: string; resume?: string | true }) => {
      ...
      if (!isInteractive({ input: process.stdin, output: process.stdout })) {
        process.stderr.write('dshline: needs a terminal on stdin and stdout; for a piped or scripted run use --profile headless\n')
        exit(1)
        return
      }
