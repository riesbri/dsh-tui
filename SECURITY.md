...
| No version younger than 24 hours is installed | `minimumReleaseAge`, `pnpm-workspace.yaml`; the one narrow exception is the Released Harness compatibility job in `.github/workflows/ci.yml` — read-only, credential-free, `--ignore-scripts` throughout, nothing ever committed or pushed — which exists to see a new Harness line immediately, so the bypass covers every pnpm command in that one disposable job, install, build, typecheck, test, and its nested capability checks alike, not just a single install step. Core, Minimum, Edge, and `harness-sync` are separate jobs that never see it, so they keep the full 24-hour brake |
