...
**两次同时进行的首次运行是 Harness 层面的问题，本仓库不用自己的锁去回答它。**针对真实启动器实测：对同一个全新配置文件的两次 `dsh plugin` 调用可能都成功，也可能让其中一个读到另一个进程正写到一半的配置文件清单——`readProfileManifest` 抛出 `SyntaxError: Unexpected end of JSON input`，一个进程崩溃而另一个完成（在一台机器上的八次双进程试验中出现三次；清单是用普通的 `writeFileSync` 写入的，所以这种读到半截的情况是固有的，而不是偶发）。因此 dshline 的份额既小又刻意：它会在问题显示期间重新检查配置文件是否已被初始化，从而关闭常见的时间窗——一个启动器等一个人回答是很长的时间——其余情况就让失败的安装失败，连同 Harness 的诊断和一行提示再试一次。在 `$DSH_HOME` 下再加一层软件包管理器的锁，正是本文档禁止的竞争性生命周期；清单的原子写入与变更加锁属于上游，紧挨着拥有该文件的代码。
