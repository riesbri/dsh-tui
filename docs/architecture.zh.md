...
Released 还会把当前已发布的产品线与上游仓库最新的 `dsh-v*` 标签相比较——DeepSeek 会在产品线到达 npm 之前先在 GitHub 上打标签，因此这是唯一能看到这一差距的方式。比较本身在每次触发时都会运行；把该标签检出并构建则只保留给每日计划任务与手动分发，并且和 Edge 一样保持不阻塞——一个尚未发布的标签同样还不是任何消费方能够安装的东西。
