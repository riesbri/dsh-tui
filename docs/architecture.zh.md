...
所以 `src/setup/` 在已组合的 Host 内运行，而 `dshline --setup` 保留它既有的含义（把这个包安装进 profile）。当 `ctx.llm.listProviders()` 为空时，该流程在第一次附着之前自动打开；`/setup` 则随时按需打开它。

**那个触发条件就是它的全部状态。**零个已注册路由恰好等于「`/model` 无可提供」，因此一次能发送回合的启动永远看不到这个流程，而一次不能的启动会打开修复它的东西，而不是一个提交时会失败的输入框。它的代价是一次同步的注册表读取，没有网络。任何地方都刻意没有首次运行标记：一个存下来的「已完成设置」标志是可能与它所声称描述的配置不一致的重复状态，而每次启动都重新询问注册表则不会。

setup 贡献的只是一次读取与一个顺序，此外没有别的。每一步都交给一个本来就是该事权威的浏览器——`/connect` 负责配置与认证，`/model` 负责选择——因此这里没有第二个路由编辑器、没有第二份模型目录，也没有状态机：循环每一轮都重新读取 Harness 并提供此刻为真的东西，这也是为什么中途退出不留下任何东西，运行两次与运行一次结果相同。
...
- **不一致会给出两个版本与两条对齐命令。**选择该哪一侧移动意味着判定哪个版本更新，而那正是本项目刻意不去拥有的版本兼容引擎。
