...
`userQuestions` 是这套雷达第一次证明它能发现真实的破坏：Harness 的 `ctx.userQuestions` 注册方式发生了变化，`packages/dshline/src/questions.ts` 目前仍用一个小的运行时判断把两种形态桥接起来。在上述策略下，这个桥接属于债务而不是范式：它早于"一次只支持一代"的规则，下一次采纳应当删除它，而不是扩展它。不应再写出同类的新桥接——迁移是移除旧调用，而不是同时保留两者。