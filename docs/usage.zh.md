...
列表保持每项一行；`↵` 为选中行打开一个详情层，只显示 Harness 实际发布的那些事实：对 subagent 是提供方、标签、模式（`continuable`/`one-shot`）、持久会话 id、会话驻留状态、是否还有子 subagent、生命周期 run id、本次运行是否发布了进程内子代理，以及它是本会话的直接子级；对任务是种类、id、生命周期状态、生产者细节与所有者。`↑`/`↓` 滚动放不下的详情，`esc` 返回列表；再按 `esc` 关闭 Work，会话记录保持原样。
