...
**顶部那个数字是下一次请求的提示词用量，不是会话总计。**它是提供方自己对上一次
收到的提示词的计数，加上此后对话增减部分的估算。`~` 说的正是这件事：只要那个估算
部分不为零它就出现，而在一次请求刚结束、数字完全属于提供方的那一刻它不出现。如果
没有任何路线声明过上下文窗口，就既没有百分比也没有条形图——因为没有一个如实可用的
分母可以去除。
...
交流不在其中：代表它的摘要在，名为 `compaction summary`。被缩短以便放得下的工具
结果标记为 `reduced`，这值得知道，因为你滚动缓冲区里的那张卡片仍然显示完整内容，
而模型已经看不到它了。

`↵` 打开一个条目：它是哪一类上下文、占了窗口多少、来自会话的什么位置，以及模型实际
携带内容的有界预览。
...
那四个 token 数字是 Harness 自己的记账，按提供方报告它们时所用的分桶，并且是整个
会话的累计——包括那些其消息此后已被压缩替换、但你仍然付过费的请求。`cache read
share` 特意不叫命中率：它是一个分桶除以提示词总量，而这里没有任何提供方发布过可供
比较的命中率。金额是本界面的估算，按下面的费率。
