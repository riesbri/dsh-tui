...
| `/themes` | 选择颜色配色。接受一个名字（`/themes ember`）或打开选择器 |
...
`/themes` 选择本窗口绘制所用的配色。指定名字即可切换，或不带参数运行以获得一个带说明的列表：
...
**主题只影响新的行。**完成的输出会被提交到你真实的终端滚动缓冲区并且永不重写，因此输入框上方的一切都保留它被打印时的颜色。正是这条规则让你能够正常滚动、选择与复制，主题无法豁免于它。为了仍然让变化可见，应用一套配色会在较旧的行下方直接提交一小段示例——一条提示、一条回复、一张带 diff 的工具卡片、一次失败以及状态行——以便两者并排对比。
...
在检视器内部，`ctrl-o` 退回到它前面的那张卡片，标题会数出你所在的位置（`Tool output 2/6`）。最近十二张被截断的卡片就这样保持可达，因此你滚动越过的某个结果，不会被其后的工具调用夺走；步进在最旧的一张停下，而不是绕回。每张卡片只提供一次：在最新的那张未看过的卡片之后，`ctrl-o` 回到细节循环，这正是让那个开关始终只差一次按键的原因。状态行在一轮进行中列出 `ctrl-o output`。
...
两个细节值得知道。`apiKeyEnv` 是一个*引用*，每次请求解析，因此密钥本身绝不进入文件。而它进入 `settings.yaml` 而不是 `cordis.patch.yml`——设置文档是适配器监听的层，因此路由随你保存出现与消失，无需重启。价格是反过来的：它们从 `cordis.patch.yml` 中的 `dshline` 行读取，因为本前端没有自己的设置节。
