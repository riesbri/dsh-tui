...
3. **Colour is chosen by role, never by name, and the closer is always the full reset.** `paint(text, 'error')`, not `style(text, 'red')` — a colour named at a call site is a decision no palette can revisit, which is how a failed tool and a deleted diff line ended up unable to differ. `style` stays exported as the primitive and as a test fixture, and a check fails the build if it regrows a call site. Two roles that share a colour today stay separate when they mean different things: giving them one value later is free, splitting them apart again is not. The reset at the end must stay `CSI 0 m`; the foreground-only `CSI 39 m` renders the same and is read as an OPENING sequence by every wrap, which replays it onto each continuation row and never clears.
