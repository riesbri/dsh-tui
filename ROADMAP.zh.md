...
- **MCP 服务器没有自己的浏览器。**`@deepseek-ai/dsh-mcp-client` 把每个服务器的工具注册到 `ctx.tools` 上，因此它们作为普通卡片渲染，这里无需任何工作——但它不发布任何注册表，所以没有权威可以询问哪些服务器已配置、已连接或正在失败。`/plugins` 指出每一行所声明的服务器与传输方式，因为那本就在它已经读取的组合文件里；再多一步就意味着从工具名的 `mcp__<server>__` 前缀推断能力状态，而那是一个非目标。上游提供一个注册表才能让真正的浏览器成为可能。
