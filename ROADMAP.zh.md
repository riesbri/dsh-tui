...
- **主题只影响新的行。**`/themes` 在五套随包发布的配色中选择并重绘活动区域；已提交到原生滚动缓冲区的行保留它们被打印时的颜色，因为已提交的输出永不重写。应用一套配色会在旧行下方提交一段示例，以便两者对比。尚不支持用户自定义配色：主题所依据的角色词汇仍然是内部的，在真实配色充分检验它之前就公开它，等于冻结一份未经验证的契约。
